...
extern const int RedirectionLimit;
...
// You can use [SvnASIHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
@implementation SvnASIHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
    //modified for use SVN by l00174413
	if (self == [SvnASIHTTPRequest class]) {
...
- (id)initWithURL:(NSURL *)newURL
{
    ...
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[self setTimeOutSeconds:[SvnASIHTTPRequest defaultTimeOutSeconds]];
...
{
    //modified for use SVN by l00174413
	SvnASIHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)main
{
	@try {
        ...
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		if ([SvnASIHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!tempUserAgentString) {
            //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
			tempUserAgentString = [SvnASIHTTPRequest defaultUserAgentString];
		}
		if (tempUserAgentString) {
...
//            if(!svnHttpStream)
//            {
//                svnHttpStream = [[SvnASIHttpStream alloc] init];
//            }
            
            [self setReadStream:[SvnASIHttpStream getReadStreamForStreamedHttpRequest:request stream:[self postBodyReadStream]]];
...
//                }
                ...
                [self setReadStream:[SvnASIHttpStream getReadStreamForHttpRequest:request]];
            }
		}
	}
	...
	//
	//modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest expirePersistentConnections];

	[connectionsLock lock];
	
	//add for use SVN by l00174413
	if (![[self url] host] || ![[self url] scheme] || useSVN) {
...
//        }
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
            ...
            //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
			[SvnASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
            //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
			[SvnASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
            ...
            {
                NSNumber* sentBytes = [[self readStream] propertyForKey:(NSString *)kCFStreamPropertyHTTPRequestBytesWrittenCount];
                [self setTotalBytesSent:[sentBytes unsignedLongLongValue]];
            }
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
                //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
				[SvnASIHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
    ...
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[SvnASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[SvnASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	//modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[SvnASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[SvnASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[SvnASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[SvnASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
    ...
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[SvnASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[SvnASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
        NSMethodSignature *cbSignature = [SvnASIHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
- (void)readResponseHeaders
{
    ...
    if(!useSVN)
    {
        message = (CFHTTPMessageRef)CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPResponseHeader);
       
    }
    else
    {
        message = [[self readStream] readHttpResponseHeader];
    }
...
//    NSDictionary * allHeaders = [NSMakeCollectable(CFHTTPMessageCopyAllHeaderFields(message)) autorelease];
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
            //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
			[SvnASIHTTPRequest addSessionCookie:cookie];
		}
	}
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		[SvnASIHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		[SvnASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	if ((!user || !pass)) {
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		NSURLCredential *authenticationCredentials = [SvnASIHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		NSURLCredential *authenticationCredentials = [SvnASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
...
{
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[SvnASIHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
...
- (void)handleBytesAvailable
{
	if (![self responseHeaders]) {
        ...
        NSLog(@"readResponseHeaders finished");
	}
    ...
    // added for use SVN by l00174413
    if (!useSVN)
    {
    ...
    //NSLog(@"asi read %d bytes data", bytesRead);
	...
	// If zero bytes were read, wait for the EOF to come.
        ...
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		[SvnASIHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
		...
		//Otherwise, let's add the data to our in-memory store
			...
			if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
                ...
                NSLog(@"read data:%@", [[NSString alloc]initWithData:result encoding:NSUTF8StringEncoding]);
...
{	
    ...
    NSLog(@"Response Headers1:%@", [self responseHeaders]);
...

{
    ...
    {
        NSError *underlyingError = [[self readStream] streamError];
        if (![self error]) { // We may already have handled this error
...
- (void)fetchPACFile
{
    ...
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	ASIHTTPRequest *PACRequest = [SvnASIHTTPRequest requestWithURL:[self PACurl]];
...
- (void)stream:(NSStream *)stream handleEvent:(NSStreamEvent)eventCode
{
    ...
    {
        [self handleNetworkEvent:eventCode];
        return;
    }
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    //modified for use SVN by l00174413
	SvnASIHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
    [newRequest setUseSVN:[self useSVN]];
...
+ (NSMutableArray *)sessionCookies
{
	...
	if (!sessionCookies) {
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		[SvnASIHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSUInteger i;
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	NSUInteger max = [[SvnASIHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[SvnASIHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[SvnASIHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
    
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[[SvnASIHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
- (void)performThrottling
{
    ...
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest measureBandwidthUsage];
	if ([SvnASIHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		[SvnASIHTTPRequest recordBandwidthUsage];
	}
...
+ (void)setShouldThrottleBandwidthForWWAN:(BOOL)throttle
{
	if (throttle) {
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		[SvnASIHTTPRequest throttleBandwidthForWWANUsingLimit:ASIWWANBandwidthThrottleAmount];
	} else {
        //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
		[SvnASIHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[SvnASIHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
    ...
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	[SvnASIHTTPRequest reachabilityChanged:nil];
}
...
+ (void)reachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
    //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
	isBandwidthThrottled = [SvnASIHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (NSDate *)expiryDateForRequest:(ASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
            //modified for use SVN by l00174413, use SvnASIHTTPRequest to call class method
			return [SvnASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
#pragma override property getter/setter
//@property (assign) BOOL complete;
- (BOOL) complete
{
    return complete;
...
- (void) setResponseCookies:(NSArray *)newResponseCookies
{
    if(responseCookies != newResponseCookies)
    {
        [responseCookies release];
        responseCookies = [newResponseCookies retain];
    }    
}
...
- (void) setResponseStatusCode:(int)newResponseStatusCode
{
...
- (void) setLastActivityTime:(NSDate *)newLastActivityTime
{
    if(lastActivityTime != newLastActivityTime)
    {
        [lastActivityTime release];
        [newLastActivityTime retain];
...
{
    return partialDownloadSize;
}

- (void) setPartialDownloadSize:(unsigned long long)newPartialDownloadSize
{
   partialDownloadSize = newPartialDownloadSize;
}
...
- (void) setUploadBufferSize:(unsigned long long)newUploadBufferSize
{
    uploadBufferSize = newUploadBufferSize;
}

//@property (retain, nonatomic) NSOutputStream *postBodyWriteStream;
- (NSOutputStream *) postBodyWriteStream
{
    return postBodyWriteStream;
}

- (void) setPostBodyWriteStream:(NSOutputStream *)newPostBodyWriteStream
{
    if(postBodyWriteStream != newPostBodyWriteStream)
    {
        [postBodyWriteStream release];
        [newPostBodyWriteStream retain];
        postBodyWriteStream  = newPostBodyWriteStream;
    }
}
...
{
    return postBodyReadStream;
}

- (void) setPostBodyReadStream:(NSInputStream *)newPostBodyReadStream
{
    if(postBodyReadStream != newPostBodyReadStream)
    {
        [postBodyReadStream release];
        [newPostBodyReadStream retain];
        postBodyReadStream  = newPostBodyReadStream;
    }
}
...
- (void) setLastBytesRead:(unsigned long long)newLastBytesRead
{
...
- (void) setLastBytesSent:(unsigned long long)newLastBytesSent
{
    lastBytesSent = newLastBytesSent;
}
...
- (void) setCancelledLock:(NSRecursiveLock *)newCancelledLock
{
    if(cancelledLock != newCancelledLock)
    {
        [cancelledLock release];
        [newCancelledLock retain];
        cancelledLock  = newCancelledLock;
    }
}
...
- (void) setFileDownloadOutputStream:(NSOutputStream *)newFileDownloadOutputStream
{
    if(fileDownloadOutputStream != newFileDownloadOutputStream)
    {
        [fileDownloadOutputStream release];
        [newFileDownloadOutputStream retain];
        fileDownloadOutputStream  = newFileDownloadOutputStream;
    }
}
...
{
    return inflatedFileDownloadOutputStream;
}

- (void) setInflatedFileDownloadOutputStream:(NSOutputStream *)newInflatedFileDownloadOutputStream
{
    if(inflatedFileDownloadOutputStream != newInflatedFileDownloadOutputStream)
    {
        [inflatedFileDownloadOutputStream release];
        [newInflatedFileDownloadOutputStream retain];
        inflatedFileDownloadOutputStream  = newInflatedFileDownloadOutputStream;
    }
}
...
- (void) setAuthenticationRetryCount:(int)newAuthenticationRetryCount
{
    authenticationRetryCount = newAuthenticationRetryCount;
}

//@property (assign) int proxyAuthenticationRetryCount;
- (int) proxyAuthenticationRetryCount
{
    return proxyAuthenticationRetryCount;
}

- (void) setProxyAuthenticationRetryCount:(int)newProxyAuthenticationRetryCount
{
    proxyAuthenticationRetryCount = newProxyAuthenticationRetryCount;
}
...
- (void) setUpdatedProgress:(BOOL)newUpdatedProgress
{
...
- (void) setNeedsRedirect:(BOOL)newNeedsRedirect
{
    needsRedirect = newNeedsRedirect;
}
...
- (void) setRedirectCount:(int)newRedirectCount
{
...
{
    return compressedPostBody;
}

- (void) setCompressedPostBody:(NSData *)newCompressedPostBody
{
    if(compressedPostBody != newCompressedPostBody)
    {
        [compressedPostBody release];
        [newCompressedPostBody retain];
        compressedPostBody  = newCompressedPostBody;
    }
}
...
{
    return compressedPostBodyFilePath;
}

- (void) setCompressedPostBodyFilePath:(NSString *)newCompressedPostBodyFilePath
{
    if(compressedPostBodyFilePath != newCompressedPostBodyFilePath)
    {
        [compressedPostBodyFilePath release];
        [newCompressedPostBodyFilePath retain];
        compressedPostBodyFilePath  = newCompressedPostBodyFilePath;
    }
}
...
- (void) setAuthenticationRealm:(NSString *)newAuthenticationRealm
{
    if(authenticationRealm != newAuthenticationRealm)
    {
        [authenticationRealm release];
        [newAuthenticationRealm retain];
        authenticationRealm  = newAuthenticationRealm;
    }
}
...
- (void) setProxyAuthenticationRealm:(NSString *)newProxyAuthenticationRealm
{
    if(proxyAuthenticationRealm != newProxyAuthenticationRealm)
    {
        [proxyAuthenticationRealm release];
        [newProxyAuthenticationRealm retain];
        proxyAuthenticationRealm  = newProxyAuthenticationRealm;
    }
}
...
- (void) setResponseStatusMessage:(NSString *)newResponseStatusMessage
{
    if(responseStatusMessage != newResponseStatusMessage)
    {
        [responseStatusMessage release];
        [newResponseStatusMessage retain];
        responseStatusMessage  = newResponseStatusMessage;
    }
}
...
{
    return willRetryRequest;
}

- (void) setWillRetryRequest:(BOOL)newWillRetryRequest
{
    willRetryRequest = newWillRetryRequest;
}

//@property (assign) BOOL connectionCanBeReused;
- (BOOL) connectionCanBeReused
{
    return connectionCanBeReused;
}

- (void) setConnectionCanBeReused:(BOOL)newConnectionCanBeReused
{
    connectionCanBeReused = newConnectionCanBeReused;
}
...
- (void) setConnectionInfo:(NSMutableDictionary *)newConnectionInfo
{
    if(connectionInfo != newConnectionInfo)
    {
        [connectionInfo release];
        [newConnectionInfo retain];
...
- (void) setReadStream:(NSInputStream *)newReadStream
{
    if(readStream != newReadStream)
    {
        [readStream release];
        [newReadStream retain];
...
- (void) setAuthenticationNeeded:(ASIAuthenticationState)AuthenticationNeeded
{
...
{
    return readStreamIsScheduled;
}

- (void) setReadStreamIsScheduled:(BOOL)newReadStreamIsScheduled
{
    readStreamIsScheduled = newReadStreamIsScheduled;
}
...
- (void) setDownloadComplete:(BOOL)newDownloadComplete
{
    downloadComplete = newDownloadComplete;
}
...
- (void) setRequestID:(NSNumber *)newRequestID
{
...
- (void) setRunLoopMode:(NSString *)newRunLoopMode
{
    if(runLoopMode != newRunLoopMode)
    {
        [runLoopMode release];
        [newRunLoopMode retain];
        runLoopMode  = newRunLoopMode;
    }
}
...
//- (void) setStatusTimer:(NSTimer *)newStatusTimer
//{
//    if(statusTimer != newStatusTimer)
//    {
//        [statusTimer release];
//        [newStatusTimer retain];
//        statusTimer  = newStatusTimer;
//    }
//}
...
- (void) setDidUseCachedResponse:(BOOL)newDidUseCachedResponse
{
    didUseCachedResponse = newDidUseCachedResponse;
}
...
- (void) setRedirectURL:(NSURL *)newRedirectURL
{
    if(redirectURL != newRedirectURL)
    {
        [redirectURL release];
        [newRedirectURL retain];
...
{
    return isPACFileRequest;
}

- (void) setIsPACFileRequest:(BOOL)newIsPACFileRequest
{
    isPACFileRequest = newIsPACFileRequest;
}

//@property (retain, nonatomic) ASIHTTPRequest *PACFileRequest;
- (ASIHTTPRequest *) PACFileRequest
{
    return PACFileRequest;
}

- (void) setPACFileRequest:(ASIHTTPRequest *)newPACFileRequest
{
    if(PACFileRequest != newPACFileRequest)
    {
        [PACFileRequest release];
        [newPACFileRequest retain];
        PACFileRequest  = newPACFileRequest;
    }
}
...
{
    return PACFileReadStream;
}

- (void) setPACFileReadStream:(NSInputStream *)newPACFileReadStream
{
    if(PACFileReadStream != newPACFileReadStream)
    {
        [PACFileReadStream release];
        [newPACFileReadStream retain];
        PACFileReadStream  = newPACFileReadStream;
    }
}
...
- (void) setPACFileData:(NSMutableData *)newPACFileData
{
    if(PACFileData != newPACFileData)
    {
        [PACFileData release];
        [newPACFileData retain];
        PACFileData  = newPACFileData;
    }
}
...
- (void) setSynchronous:(BOOL)newSynchronous
{
    isSynchronous = newSynchronous;
}
