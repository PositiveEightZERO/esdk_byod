...
@property (strong, nonatomic) APLCustomAVARLDelegate * resourceDelegate;
...
@synthesize closeButton, playButton, fullscreenButton,  progressIndicator, currentTimeLabel, remainingTimeLabel, spacerView; //volumeView,
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    NSLog(@"defaultFrame;%@", NSStringFromCGRect(defaultFrame));
    [self setup];
    return self;
}
...
//                                     @"V" : volumeView,
    ...
    [closeButton setImage:[UIImage imageNamed:@"gui_close"] forState:UIControlStateNormal];
...
- (void)refreshProgressIndicator {
    ...
    if (duration == 0 || isnan(duration)) {
        ...
        CGFloat current = seeking ? progressIndicator.value * duration :   CMTimeGetSeconds(player.currentTime);      // If seeking, reflects the position of the slider , Otherwise, use the actual video position
...
- (void)prepareAndPlayAutomatically:(BOOL)playAutomatically {
    ...
    self.resourceDelegate = [[APLCustomAVARLDelegate alloc] init];
    ...
    [currentItem addObserver:self forKeyPath:@"playbackLikelyToKeepUp" options:0 context:nil];
