 ...
 * 
 * <p>You create a subclass implementing {@link #onCreate}, {@link #onUpgrade}
 * and optionally {@link #onOpen}, and this class takes care of opening the
 * database if it exists, creating it if it does not, and upgrading it as
 * necessary. Transactions are used to make sure the database is always in a
 * sensible state.
 * 
 ...
 * 
 * <p>For an example, see the NotePadProvider class in the NotePad sample
 * application, in the <em>samples/</em> directory of the SDK.</p>
 * 
 * <p class="note"><strong>Note:</strong> this class assumes monotonically
 * increasing version numbers for upgrades.</p>
 */
@SuppressLint("NewApi")
public abstract class SQLiteOpenHelper
{
     ...
     */
    public SQLiteDatabase getWritableDatabase(String username)
    {
        synchronized (this)
        {
            return getDatabaseLocked(username, true);
        }
    }
     ...
     */
    public SQLiteDatabase getReadableDatabase(String username)
    {
        synchronized (this)
        {
            return getDatabaseLocked(username, false);
        }
    }

    private SQLiteDatabase getDatabaseLocked(String username, boolean writable)
    {
        if (mDatabase != null)
        {
            if (!mDatabase.isOpen())
        ...
        try
        {
            ...
            else if (mName == null)
            {
                ...
                //db = null;
                throw new IllegalArgumentException("database name null");
            }
            else
            {
...
//                    else
//                    {
//                        int flags = mEnableWriteAheadLogging ? SQLiteDatabase.ENABLE_WRITE_AHEAD_LOGGING
//                                : 0;
                        db = SQLiteDatabase
                                .openDatabase(
                                        username,
                                        path,
                                        mFactory, SQLiteDatabase.CREATE_IF_NECESSARY, mErrorHandler);
...
//                    }
                ...
                catch (SQLiteException ex)
                {
                    ...
                    db = SQLiteDatabase.openDatabase(username, path, mFactory,
                            SQLiteDatabase.OPEN_READONLY, mErrorHandler);
                }
            }
     ...
     * features such as write-ahead logging or foreign key support. <p> This
     * method is called before {@link #onCreate}, {@link #onUpgrade},
     * {@link #onDowngrade}, or {@link #onOpen} are called. It should not modify
     * the database except to configure the database connection as required.
     * </p><p> This method should only call methods that configure the
     * parameters of the database connection, such as
     * {@link SQLiteDatabase#enableWriteAheadLogging}
     * {@link SQLiteDatabase#setForeignKeyConstraintsEnabled},
     * {@link SQLiteDatabase#setLocale}, {@link SQLiteDatabase#setMaximumSize},
     * or executing PRAGMA statements. </p>
     * 
     * @param db
     *            The database.
     */
     ...
     * contents of the old table. </p><p> This method executes within a
     * transaction. If an exception is thrown, all changes will automatically be
     * rolled back. </p>
     * 
     * @param db
     *            The database.
     ...
     * 
     * <p> This method executes within a transaction. If an exception is thrown,
     * all changes will automatically be rolled back. </p>
     * 
     * @param db
     *            The database.
     * @param oldVersion
     ...
     * {@link SQLiteDatabase#isReadOnly} before updating the database. <p> This
     * method is called after the database connection has been configured and
