 ...
 * 
 * <p>A side effect of reading an HTTP response is that the response cache is
 * populated. If the stream is closed early, that cache entry will be
 * invalidated.
 * 
 * @author l00174413
 * @version 1.0
...
public abstract class AbstractHttpInputStream extends InputStream
{
    ...
    /**
     * Instantiates a new abstract http input stream.
     * 
     * @param in
     *            the in
    ...
    AbstractHttpInputStream(InputStream in,
            SvnHttpURLConnection httpURLConnection, CacheRequest cacheRequest)
            throws IOException
    {
        this.in = in;
        this.httpURLConnection = httpURLConnection;
        this.cacheRequest = cacheRequest;
        this.cacheOut = (null != cacheRequest ? cacheRequest.getBody() : null);
    }
    ...
    /**
     * skip(long) is implemented using read(byte[], int, int) so subclasses only
     * need to override the latter.
     * 
     * @param n
     *            the n
    ...
    public final long skip(long n) throws IOException
    {
        ...
        while (total < n)
        {
            // Calling read() ensures the skipped bytes make it into the
            // response cache.
            count = read(skipBuffer, 0,
                    (int) Math.min(n - total, skipBuffer.length));
            if (count == -1)
            {
                break;
            }
     ...
     *              I/O异常
     */
    protected final void cacheWrite(byte[] buffer, int offset, int count)
            throws IOException
    {
        if (null != cacheOut)
        {
    ...
    /**
     * Calls abort on the cache entry and disconnects the socket. This should be
     * invoked when the connection is closed unexpectedly to invalidate the
     * cache entry and to prevent the HTTP connection from being reused. HTTP
     * messages are sent in serial so whenever a message cannot be read to
     * completion, subsequent messages cannot be read either and the connection
     * must be discarded.
     * 
     * <p>An earlier implementation skipped the remaining bytes, but this
     * requires that the entire transfer be completed. If the intention was to
     * cancel the transfer, closing the connection is the only solution.
     */
