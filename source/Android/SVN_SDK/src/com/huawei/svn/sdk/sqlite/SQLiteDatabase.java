...
import com.huawei.svn.sdk.sqlite.SQLiteDebug.DbStats;
 ...
 * perform other common database management tasks. </p><p> See the Notepad
 * sample application in the SDK for an example of creating and managing a
 * database. </p><p> Database names must be unique within an application, not
 * across all applications. </p>
 * 
 * <h3>Localized Collation - ORDER BY</h3> <p> In addition to SQLite's default
 * <code>BINARY</code> collator, Android supplies two more,
 * <code>LOCALIZED</code>, which changes with the system's current locale, and
 * <code>UNICODE</code>, which is the Unicode Collation Algorithm and not
 * tailored to the current locale. </p>
 */
@SuppressLint("NewApi")
public final class SQLiteDatabase extends SQLiteClosable
{
    ...
    //
    // The lock MUST NOT be held while attempting to acquire database
    // connections or
    // while executing SQL statements on behalf of the client as it can lead to
    // deadlock.
    //
    // It is ok to hold the lock while reconfiguring the connection pool or
    // dumping
    // statistics because those operations are non-reentrant and do not try to
    // acquire
    // connections that might be held by other threads.
    //
    // Basic rule: grab the lock, access or modify global state, release the
    // lock, then
    ...
    // The pool itself is thread-safe, but the reference to it can only be
    // acquired
    ...
    private SQLiteDatabase(String username, String path, int openFlags,
            CursorFactory cursorFactory, DatabaseErrorHandler errorHandler)
    {
        ...
        mConfigurationLocked = new SQLiteDatabaseConfiguration(path, username,
                openFlags);
    }
    ...
    private static boolean isMainThread()
    {
        ...
        // Would also be nice to have something that would work across Binder
        // calls.
     ...
     * transaction and all of the nested transactions will be committed or
     * rolled back. The changes will be rolled back if any transaction is ended
     * without being marked as clean (by calling setTransactionSuccessful).
     * Otherwise they will be committed. </p> <p>Here is the standard idiom for
     * transactions:
     * 
     * <pre> db.beginTransaction(); try { ... db.setTransactionSuccessful(); }
     * finally { db.endTransaction(); } </pre>
     */
     ...
     * will be committed. <p> Here is the standard idiom for transactions:
     * 
     * <pre> db.beginTransactionNonExclusive(); try { ...
     * db.setTransactionSuccessful(); } finally { db.endTransaction(); } </pre>
     */
     ...
     * transactions:
     * 
     * <pre> db.beginTransactionWithListener(listener); try { ...
     * db.setTransactionSuccessful(); } finally { db.endTransaction(); } </pre>
     * 
     ...
     * will be committed. <p> Here is the standard idiom for transactions:
     * 
     * <pre> db.beginTransactionWithListenerNonExclusive(listener); try { ...
     * db.setTransactionSuccessful(); } finally { db.endTransaction(); } </pre>
     * 
     * @param transactionListener
     *            listener that should be notified when the transaction begins,
     ...
     * database. <p> The name of this method comes from a time when having an
     * active connection to the database meant that the thread was holding an
     * actual lock on the database. Nowadays, there is no longer a true
     * "database lock" although threads may block if they cannot acquire a
     * database connection to perform a particular operation. </p>
     * 
    ...
    /**
     * Always returns false. <p> There is no longer the concept of a database
     * lock, so this method always returns false. </p>
     * 
     * @return False.
    ...
    private boolean yieldIfContendedHelper(boolean throwIfUnsafe,
            long sleepAfterYieldDelay)
    {
        acquireReference();
        try
        {
            return getThreadSession().yieldTransaction(sleepAfterYieldDelay,
                    throwIfUnsafe);
        }
        finally
        {
     ...
     */
    public static SQLiteDatabase openDatabase(String username, String path,
            CursorFactory factory, int flags)
    {
        return openDatabase(username, path, factory, flags, null);
    }
     ...
     * {@link DatabaseErrorHandler} to be used to handle corruption when sqlite
    ...
    {
        SQLiteDatabase db = new SQLiteDatabase(username, path, flags, factory,
                errorHandler);
        db.open();
        return db;
    }
     ...
     */
    public static SQLiteDatabase openOrCreateDatabase(String username,
            File file, CursorFactory factory)
    {
        return openOrCreateDatabase(username, file.getPath(), factory);
    }
     ...
     */
    public static SQLiteDatabase openOrCreateDatabase(String username,
            String path, CursorFactory factory)
    {
        return openDatabase(username, path, factory, CREATE_IF_NECESSARY, null);
    }
    ...
    {
        return openDatabase(username, path, factory, CREATE_IF_NECESSARY,
                errorHandler);
    }
...
//            
    ...
    private void openInner()
    {
        synchronized (mLock)
        {
            ...
            mConnectionPoolLocked = SQLiteConnectionPool
                    .open(mConfigurationLocked);
     ...
     * @hide
     */
    public void addCustomFunction(String name, int numArgs,
            CustomFunction function)
    {
        // Create wrapper (also validates arguments).
        SQLiteCustomFunction wrapper = new SQLiteCustomFunction(name, numArgs,
                function);
    ...
    {
        return ((Long) DatabaseUtils.longForQuery(this, "PRAGMA user_version;",
                null)).intValue();
    }
    ...
    {
        long pageCount = DatabaseUtils.longForQuery(this,
                "PRAGMA max_page_count;", null);
    ...
    public long setMaximumSize(long numBytes)
    {
        ...
        long newPageCount = DatabaseUtils.longForQuery(this,
                "PRAGMA max_page_count = " + numPages, null);
     ...
     * larger than 1x1. <p> No two threads should be using the same
     * {@link SQLiteStatement} at the same time.
     * 
     * @param sql
    ...
    {
        return queryWithFactory(null, distinct, table, columns, selection,
...
//    {
//        return queryWithFactory(cursorFactory, distinct, table, columns,
            ...
            ContentValues initialValues, int conflictAlgorithm)
    {
        ...
        try
        {
            ...
            if (size > 0)
            {
                ...
                // solution for API < 11
//                for (Entry<String, Object> item : initialValues.valueSet())
//                {
    ...
    public int delete(String table, String whereClause, String[] whereArgs)
    {
        ...
        try
        {
            SQLiteStatement statement = new SQLiteStatement(this,
                    "DELETE FROM "
                            + table
                            + (!TextUtils.isEmpty(whereClause) ? " WHERE "
...
//            }
     ...
     * statement that returns data. <p> It has no means to return any data (such
     * as the number of affected rows). Instead, you're encouraged to use
     * {@link #insert(String, String, ContentValues)},
     * {@link #update(String, ContentValues, String, String[])}, et al, when
     * possible. </p> <p> When using {@link #enableWriteAheadLogging()},
     * journal_mode is automatically managed by this class. So, do not set
     * journal_mode using "PRAGMA journal_mode'<value>" statement if your app is
     * using {@link #enableWriteAheadLogging()} </p>
     * 
     * @param sql
     ...
     * <p> For INSERT statements, use any of the following instead. <ul> <li>
     * {@link #insert(String, String, ContentValues)}</li> <li>
     * {@link #insertOrThrow(String, String, ContentValues)}</li> <li>
     * {@link #insertWithOnConflict(String, String, ContentValues, int)}</li>
     * </ul> <p> For UPDATE statements, use any of the following instead. <ul>
     * <li>{@link #update(String, ContentValues, String, String[])}</li> <li>
     * {@link #updateWithOnConflict(String, ContentValues, String, String[], int)}
     * </li> </ul> <p> For DELETE statements, use any of the following instead.
     * <ul> <li>{@link #delete(String, String, String[])}</li> </ul> <p> For
     * example, the following are good candidates for using this method: <ul>
     * <li>ALTER TABLE</li> <li>CREATE or DROP table / trigger / view / index /
     * virtual table</li> <li>REINDEX</li> <li>RELEASE</li> <li>SAVEPOINT</li>
     * <li>PRAGMA that returns no data</li> </ul> </p> <p> When using
     * {@link #enableWriteAheadLogging()}, journal_mode is automatically managed
     * by this class. So, do not set journal_mode using
     * "PRAGMA journal_mode'<value>" statement if your app is using
     * {@link #enableWriteAheadLogging()} </p>
     * 
     * @param sql
     *            the SQL statement to be executed. Multiple statements
     *            separated by semicolons are not supported.
     ...
     * cache). <p> Maximum cache size can ONLY be increased from its current
     * size (default = 10). If this method is called with smaller size than the
     * current maximum value, then IllegalStateException is thrown. <p> This
    ...
    /**
     * Sets whether foreign key constraints are enabled for the database. <p> By
     * default, foreign key constraints are not enforced by the database. This
     * method allows an application to enable foreign key constraints. It must
     * be called each time the database is opened to ensure that foreign key
     * constraints are enabled for the session. </p><p> A good time to call this
     * method is right after calling {@link #openOrCreateDatabase} or in the
     * {@link SQLiteOpenHelper#onConfigure} callback. </p><p> When foreign key
     * constraints are disabled, the database does not check whether changes to
     * the database will violate foreign key constraints. Likewise, when foreign
     * key constraints are disabled, the database will not execute cascade
     * delete or update triggers. As a result, it is possible for the database
     * state to become inconsistent. To perform a database integrity check, call
     * {@link #isDatabaseIntegrityOk}. </p><p> This method must not be called
     * while a transaction is in progress. </p><p> See also <a
     * href="http://sqlite.org/foreignkeys.html">SQLite Foreign Key
     ...
     * concurrently with reads. <p> When write-ahead logging is not enabled (the
     * default), it is not possible for reads and writes to occur on the
     * database at the same time. Before modifying the database, the writer
     * implicitly acquires an exclusive lock on the database which prevents
     * readers from accessing the database until the write is completed. </p><p>
     ...
     * perceive the new state of the database. </p><p> It is a good idea to
     * enable write-ahead logging whenever a database will be concurrently
     * accessed and modified by multiple threads at the same time. However,
     * write-ahead logging uses significantly more memory than ordinary
     * journaling because there are multiple connections to the same database.
     * So if a database will only be used by a single thread, or if optimizing
     * concurrency is not very important, then write-ahead logging should be
     * disabled. </p><p> After calling this method, execution of queries in
     * parallel is enabled as long as the database remains open. To disable
     * execution of queries in parallel, either call
     * {@link #disableWriteAheadLogging} or close the database and reopen it.
     * </p><p> The maximum number of connections used to execute queries in
     * parallel is dependent upon the device memory and possibly other
     * properties. </p><p> If a query is part of a transaction, then it is
     * executed on the same database handle the transaction was begun. </p><p>
     * Writers should use {@link #beginTransactionNonExclusive()} or
     * {@link #beginTransactionWithListenerNonExclusive(SQLiteTransactionListener)}
     * to start a transaction. Non-exclusive mode allows database file to be in
     * readable by other threads executing queries. </p><p> If the database has
     * any attached databases, then execution of queries in parallel is NOT
     * possible. Likewise, write-ahead logging is not supported for read-only
     * databases or memory databases. In such cases,
     * {@link #enableWriteAheadLogging} returns false. </p><p> The best way to
     * enable write-ahead logging is to pass the
     ...
     * <code><pre> SQLiteDatabase db =
     * SQLiteDatabase.openDatabase("db_filename", cursorFactory,
     * SQLiteDatabase.CREATE_IF_NECESSARY |
     * SQLiteDatabase.ENABLE_WRITE_AHEAD_LOGGING, myDatabaseErrorHandler);
     * db.enableWriteAheadLogging(); </pre></code> </p><p> Another way to enable
     * write-ahead logging is to call {@link #enableWriteAheadLogging} after
     * opening the database. <code><pre> SQLiteDatabase db =
     * SQLiteDatabase.openDatabase("db_filename", cursorFactory,
     * SQLiteDatabase.CREATE_IF_NECESSARY, myDatabaseErrorHandler);
     * db.enableWriteAheadLogging(); </pre></code> </p><p> See also <a
     * href="http://sqlite.org/wal.html">SQLite Write-Ahead Logging</a> for more
     * details about how write-ahead logging works. </p>
     * 
    ...
    public boolean enableWriteAheadLogging()
    {
        synchronized (mLock)
        {
            ...
            // make sure this database has NO attached databases because
    ...
    public List<Pair<String, String>> getAttachedDbs()
    {
        ...
        synchronized (mLock)
        {
            ...
            if (!mHasAttachedDbsLocked)
            {
                ...
                // There is a small window where attached databases exist but
                // this flag is not
                // set yet. This can occur when this thread is in a race
                // condition with another
                // thread that is executing the SQL statement:
                // "attach database <blah> as <foo>"
                // If this thread is NOT ok with such a race condition (and thus
                // possibly not
                // receivethe entire list of attached databases), then the
                // caller should ensure
                // that no thread is executing any SQL statements while a thread
                // is calling this
                // method. Typically, this method is called when 'adb bugreport'
                // is done or the
                // caller wants to collect stats on the database and all its
                // attached databases.
                attachedDbs.add(new Pair<String, String>("main",
                        mConfigurationLocked.path));
                return attachedDbs;
            }
        ...
        try
        {
            // has attached databases. query sqlite to get the list of attached
            // databases.
            Cursor c = null;
            try
            {
                ...
                while (c.moveToNext())
                {
                    // sqlite returns a row for each database in the returned
                    // list of databases.
                    // in each row,
                    // 1st column is the database name such as main, or the
                    // database
                    // name specified on the "ATTACH" command
                    // 2nd column is the database file path.
                    attachedDbs.add(new Pair<String, String>(c.getString(1), c
                            .getString(2)));
                }
            }
            finally
     ...
     * databases) pass integrity_check, false otherwise. <p> If the result is
     * false, then this method logs the errors reported by the integrity_check
     * command execution. <p> Note that 'pragma integrity_check' on a database
    ...
    public boolean isDatabaseIntegrityOk()
    {
        ...
        try
        {
            ...
            {
                // can't get attachedDb list. do integrity check on the main
                // database
            ...
            for (int i = 0; i < attachedDbs.size(); i++)
            {
                ...
                try
                {
                    prog = compileStatement("PRAGMA " + p.first
                            + ".integrity_check(1);");
                    String rslt = prog.simpleQueryForString();
                    if (!rslt.equalsIgnoreCase("ok"))
                    {
                        // integrity_checker failed on main or attached
                        // databases
                        Log.e(TAG, "PRAGMA integrity_check on " + p.second
                                + " returned: " + rslt);
                        return false;
                    }
                }
    ...
    {
        return super.toString() + "|SQLiteDatabase: " + getPath();
    }
    ...
    private void throwIfNotOpenLocked()
    {
        if (mConnectionPoolLocked == null)
        {
            throw new IllegalStateException("The database '"
                    + mConfigurationLocked.label + "' is not open.");
        }
    }
