...
public class SSLSocketImpl extends SSLSocket {
    ...
    private static boolean useSVN = true;
    ...
    protected SSLSocketImpl(String host, int port, SSLParametersImpl sslParameters)
        ...
        //super(host, port);
        //this.socket = new SvnSocket(host, port);
    ...
    protected SSLSocketImpl(String host, int port,
        ...
        //this.socket = new SvnSocket(host, port, localHost, localPort);
        ...
        if(useSVN)
        {
            ...
            this.socket = new Socket(host, port, localHost, localPort);
        }
    ...
    protected SSLSocketImpl(InetAddress address, int port,
            ...
            SSLParametersImpl sslParameters) throws IOException {
        //this.socket = new SvnSocket(address, port, localAddress, localPort);
        ...
        if(useSVN)
        {
            ...
            this.socket = new Socket(address, port, localAddress, localPort);
        }
...
//        output = super.getOutputStream();
    ...
    int getPeerPort() {
        return (wrappedPort == -1) ? this.socket.getPort() : wrappedPort;
    }
    ...
    private void shutdown() {
        ...
        //logger.println("SSLSocket.shutdown..");
        if (handshake_started) {
...
//            	if (logger != null) {
//            		logger.println("appDataIS.available() == 0");
//            	}
...
//            logger.println("SSLSocket.needAppData: app data len1: "
//                    + appDataIS.available());
        } catch (EndOfSourceException e) {
            // end of socket's input stream has been reached
            appDataIS.setEnd();
//            logger.println("SSLSocket.needAppData: app data len2: "
//                    + appDataIS.available());
        }
        if (logger != null) {
    ...
    protected void writeAppData(byte[] data, int offset, int len) throws IOException {
    	if (logger != null) {
    		logger.println("SSLSocket.writeAppData: begin" +len + " " + SSLRecordProtocol.MAX_DATA_LENGTH);
    	}
...
//                        data, offset, len));
                byte[] wrapped = recordProtocol.wrap(ContentType.APPLICATION_DATA,
                        data, offset, len);
    ...
    {
        // TODO Auto-generated method stub
        this.socket.setTrafficClass(value);
    }
